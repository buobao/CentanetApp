...
public class DotIndicator extends View implements ViewPager.OnPageChangeListener  {
    ...
    public DotIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = getResources().obtainAttributes(attrs, R.styleable.DotIndicator);
        //获取自定义属性的个数
        int N = typedArray.getIndexCount();
        for (int i = 0; i < N; i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.DotIndicator_indicator_icon) {
                mIndicator = typedArray.getDrawable(attr);
            } else if (attr == R.styleable.DotIndicator_indicator_margin) {
            ...
            } else if (attr == R.styleable.DotIndicator_indicator_smooth) {
